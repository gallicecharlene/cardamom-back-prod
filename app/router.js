...
router.get('/api/stats/:statsId', authJwt, errorHandler(statsController.getOne)); // Récupérer la stats par son Id (par utilisateur)
router.patch('/api/stats/:statsId', authJwt, errorHandler(statsController.update)); // route d'incrémentation depuis la page MemoTest
router.delete('/api/stats/:statsId', authJwt, errorHandler(statsController.delete)); // supprimer une stats par son Id (par utilisateur)
